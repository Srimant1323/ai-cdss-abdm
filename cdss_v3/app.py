...
def start_simulator():
    ...
    sim = BedOccupancySimulator(use_mqtt=False)  # local mode; set True for MQTT
...
try:
    ...
    pass  # Simulator optional — app runs without it

# ─────────────────────────────────────────────────────────────────────────────
# DATA LOADING
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=25)   # Re-reads CSV every 25s — picks up simulator updates
def load_beds():
    return pd.read_csv("data/BedOccupancy.csv")

@st.cache_data
def load_hospitals(): return pd.read_csv("data/Hospitals.csv")
...
def load_cases():
    ...
    df = (cases.merge(patients,  on="patient_id")
               .merge(doctors,   on="doctor_id")
               .merge(hospitals, on="hospital_id", suffixes=("","_hosp")))
    return df
...
National Medical Commission (NMC) registry IDs.
    ...
    for _,d in perf.iterrows():
        ...
        with st.expander(f"{medal} #{int(d['rank'])} **{d['name']}** — {d['specialization']} · {d['name_y']} · {d['city']}"):
